...
<div class="relative">
    <input id="{{ $name }}" name="{{ $name }}" class="form-input text-wgg-black-950 w-full px-4 py-2 pt-6 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-wgg-black-950 focus:border-wgg-border transition duration-150 ease-in-out" type="{{ $type }}" value="{{ $value }}" placeholder=" " {{ $required ? 'required' : '' }}>
